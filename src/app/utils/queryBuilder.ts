...
> {
  ...
  filter(): this {
    ...
    Object.keys(this.queryParams).forEach((key) => {
      if (!excludedField.includes(key)) {
        // Check if key matches pattern: fieldName(operator)=value
        ...
        if (operatorMatch) {
          ...
          const filterObj = filterParams[fieldName] as Record<string, unknown>;
          filterObj[operator] = this.queryParams[key];
        } else {
  ...
  async execute(): Promise<IQueryResults<T>> {
